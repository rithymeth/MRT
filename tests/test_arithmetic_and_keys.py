...
def test_modulo_by_zero_is_still_an_error():
    ...
    ''')
    assert errors == []
    assert output == ["ArithmeticError Modulo by zero.", "ArithmeticError Modulo by zero."]
...
def test_a_boolean_and_a_number_key_coexist_in_one_object():
    ...
    assert output == ["2 [1, true] [number, boolean]", "number boolean",
...
def test_boolean_keys_round_trip_through_assignment_and_iteration():
    ...
    assert output == [
        "4 [true, 1, false, 0]",
        "true boolean t", "1 number n", "false boolean f", "0 number z",
    ]
