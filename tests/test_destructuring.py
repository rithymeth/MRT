...
def test_missing_key_without_a_default_is_an_error():
    ...
    assert errors == []
    assert output == ['Runtime Error: Cannot destructure: no key "zz" in the object. [line 1]']


def test_destructuring_a_non_object_with_an_object_pattern():
    output, errors = run_mrt('func main() { try { var {a} = [1]; } catch (e) { print(e.kind, e.message); } }')
    assert errors == []
